...
export default function FormularioNuevaConsulta({ close }) {
  ...
  const onSubmit = async () => {
    ...
    const data = {
      fecha_cita: selectedDate ? selectedDate.toISOString().split("T")[0] : "",
      hora_inicio: formData.time + ":00",
      hora_fin: formData.time + ":30",
      motivo_cita: formData.consultationType,
      id_consultorio: formData.id_consultorio || 1,
      id_servicio: formData.id_servicio || 1,
      id_veterinario: formData.veterinarian,
      id_mascota: petData.id || 1, 
      peso: formData.physicalExam.weight,
      temperatura: formData.physicalExam.temperature,
      diagnostico: formData.diagnosis,
      medicamentos: formData.medications.map(m => `${m.name} (${m.dosage}, ${m.frequency})`).join(", "),
      motivo_consulta: formData.diagnosis,
      nombre_tratamiento: formData.treatment,
      descripcion_tratamiento: formData.notes,
      prueba_laboratorio: formData.labResults[0]?.test || null,
      resultado_laboratorio: formData.labResults[0]?.result || null,
      referencia_laboratorio: formData.labResults[0]?.reference || null,
      p_mensaje: '', 
      p_id_consulta: '',
      p_id_cita: '' 
    };
    ...
    try {
      const result = await PostData('http://localhost:3000/service/api/consult/register', data);
      alert("Consulta registrada correctamente. ID: " + result.data.p_id_consulta);
      close();
    } catch (err) {
