...
const Registro = ({ URL = '', imgDefault = '' }) => {
  ...
  const enviarEmail = (codigo) => {
    ...
    emailjs.send(
      ...
      (error) => {
        setNotify({
          title: 'Error',
          message: 'Hubo un error al enviar el email. Intenta de nuevo.',
          close: setNotify
        })
  ...
  const verificarCodigo = () => {
    ...
    if (codigoInput === formData.codigoVerificacion) {
      ...
      const newUser = {
        nom_per: formData.nombres,
        ape_per: formData.apellidos,
        fec_nac_per: formData.fechaNacimiento,
        tip_doc_per: formData.tipoDocumento,
        doc_per: formData.numeroDocumento,
        dir_per: formData.direccion,
        cel_per: formData.celular,
        cel2_per: formData.celular2,
        email_per: formData.email,
        password: formData.password,
        gen_per: formData.genero
      }
      SendData(newUser)
      setPaso(1)
    } else {
  ...
  const reenviarCodigo = () => {
    ...
    setNotify({
      ...
      message: `Codigo re enviado con exito a ${formData.email}`,
      close: setNotify
    })
  }
